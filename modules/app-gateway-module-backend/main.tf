...
resource "azurerm_application_gateway" "ag" {
  ...
  dynamic "backend_address_pool" {
    for_each = [for app in local.gateways[count.index].app_configuration : {
      name = lookup(app, "full_name", "${app.product}-${app.component}")
    }]
  ...
  dynamic "probe" {
    for_each = [for app in local.gateways[count.index].app_configuration : {
      ...
      host_name     = concat(lookup(app, "full_name", "${app.product}-${app.component}"),"-${var.env}.${local.gateways[count.index].gateway_configuration.host_name_suffix}")
      ssl_host_name = concat(lookup(app, "full_name", "${app.product}-${app.component}"),".${local.gateways[count.index].gateway_configuration.ssl_host_name_suffix}")
