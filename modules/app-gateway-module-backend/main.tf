...
resource "azurerm_application_gateway" "ag" {
  ...
  dynamic "probe" {
    for_each = [for app in local.gateways[count.index].app_configuration : {
      ...
      host_name     = "lookup(app, 'full_name', ${app.product}-${app.component})-${var.env}.${local.gateways[count.index].gateway_configuration.host_name_suffix}"
      ssl_host_name = "lookup(app, 'full_name', ${app.product}-${app.component}).${local.gateways[count.index].gateway_configuration.ssl_host_name_suffix}"
