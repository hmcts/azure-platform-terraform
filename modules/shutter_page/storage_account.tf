resource "azurerm_storage_account" "shutter_store" {
  ...
  name                      = lookup(var.shutter_apps[count.index], "shutter", "") == "custom" ? "hmcts${replace(replace(lookup(var.shutter_apps[count.index], "name"), "-", ""), "tribunals", "sya")}shutter${var.env}" : "hmctsdefaultshutter${var.env}"
